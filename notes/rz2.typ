...
#grid(
  columns: (1fr,) * 2,
  $
    cal(M)_(mu,nu)^(p,0) &= integral_hat(Omega) hat(T)_mu^(p,0) hat(T)_nu^(p,0) med r dif r dif z \
    cal(M)_(mu,nu)^(p,1) &= integral_hat(Omega) hat(bold(T))_mu^(p,1) dot hat(bold(T))_nu^(p,1) med r dif r dif z \
    cal(M)_(mu,nu)^(p,2) &= integral_hat(Omega) hat(T)_mu^(p,2) hat(T)_nu^(p,2) med r dif r dif z \
  $,
  $
    cal(M)_(mu,nu)^(theta,1) &= integral_hat(Omega) hat(T)_mu^(theta,1) hat(T)_nu^(theta,1) med r dif r dif z \
    cal(M)_(mu,nu)^(theta,2) &= integral_hat(Omega) hat(bold(T))_mu^(theta,2) dot hat(bold(T))_nu^(theta,2) med r dif r dif z \
    cal(M)_(mu,nu)^(theta,3) &= integral_hat(Omega) hat(T)_mu^(theta,3) hat(T)_nu^(theta,3) med r dif r dif z \
  $
)
